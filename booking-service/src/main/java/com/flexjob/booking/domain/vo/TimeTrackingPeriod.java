...
public class TimeTrackingPeriod {
    ...
    public static TimeTrackingPeriod started(LocalDateTime startTime) {
        return new TimeTrackingPeriod(startTime, null);
    }

    public static TimeTrackingPeriod completed(LocalDateTime startTime, LocalDateTime endTime) {
        return new TimeTrackingPeriod(startTime, endTime);
    }
    ...
    public double getHours() {
        if (endTime == null) {
            throw new IllegalStateException("Cannot calculate hours for active time tracking");
        }
    ...
    public String toString() {
        if (endTime == null) {
            return String.format("TimeTrackingPeriod(started at %s, ACTIVE)", startTime);
        }
        return String.format("TimeTrackingPeriod(%s to %s, %.2f hours)",
                startTime, endTime, getHours());
    }
}
