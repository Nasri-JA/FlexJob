package com.flexjob.booking.infrastructure.adapter.input.rest.controller;

import com.flexjob.booking.application.dto.command.*;
import com.flexjob.booking.application.dto.response.*;
import com.flexjob.booking.application.port.input.*;
...
public class BookingController {
    ...
    public ResponseEntity<BookingResponse> create(@RequestBody CreateBookingCommand command) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(createBookingUseCase.createBooking(command));
    }
    ...
    public ResponseEntity<TimeTrackingResponse> startTracking(
            @PathVariable Long id, @RequestBody(required = false) String notes) {
        StartTimeTrackingCommand cmd = StartTimeTrackingCommand.builder()
    ...
    public ResponseEntity<TimeTrackingResponse> endTracking(
            @PathVariable Long id, @RequestBody(required = false) String notes) {
        EndTimeTrackingCommand cmd = EndTimeTrackingCommand.builder()
                .bookingId(BookingId.of(id))
                .notes(notes)
                .build();
