package com.flexjob.booking.infrastructure.adapter.output.persistence.adapter;

import com.flexjob.booking.application.port.output.*;
...
import com.flexjob.booking.infrastructure.adapter.output.persistence.mapper.*;
import com.flexjob.booking.infrastructure.adapter.output.persistence.repository.*;
...
public class BookingPersistenceAdapter implements
        LoadBookingPort, SaveBookingPort,
        LoadTimeTrackingPort, SaveTimeTrackingPort {
    ...
    public Optional<Booking> findById(BookingId id) {
        return bookingRepository.findById(id.getValue())
    ...
    public boolean existsById(BookingId id) {
        return bookingRepository.existsById(id.getValue());
    }

    @Override
    public boolean existsByApplicationId(Long applicationId) {
        return bookingRepository.existsByApplicationId(applicationId);
    }
    ...
    public Optional<TimeTracking> findById(TimeTrackingId id) {
        return timeTrackingRepository.findById(id.getValue())
                .map(timeTrackingMapper::toDomain);
    }

    @Override
    public List<TimeTracking> findByBookingId(Long bookingId) {
        return timeTrackingRepository.findByBooking_Id(bookingId).stream()
                .map(timeTrackingMapper::toDomain)
                .collect(Collectors.toList());
    }
    ...
    public Optional<TimeTracking> findActiveByBookingId(Long bookingId) {
        return timeTrackingRepository.findByBooking_IdAndEndTimeIsNull(bookingId)
                .map(timeTrackingMapper::toDomain);
    }

    @Override
    public TimeTracking save(TimeTracking timeTracking) {
        BookingJpaEntity bookingJpa = bookingRepository.findById(timeTracking.getBookingId())
                .orElseThrow();
