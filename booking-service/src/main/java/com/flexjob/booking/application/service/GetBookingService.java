...
import com.flexjob.booking.application.port.input.GetBookingUseCase;
...
public class GetBookingService implements GetBookingUseCase {
    ...
    public BookingResponse getBookingById(BookingId id) {
        Booking booking = loadBookingPort.findById(id)
                .orElseThrow(() -> BookingNotFoundException.byId(id));
    ...
    public List<BookingResponse> getBookingsByEmployeeId(Long employeeId) {
        return loadBookingPort.findByEmployeeId(employeeId).stream()
    ...
    public List<BookingResponse> getBookingsByEmployerId(Long employerId) {
        return loadBookingPort.findByEmployerId(employerId).stream()
                .map(this::mapToResponse)
                .collect(Collectors.toList());
    }
    ...
    public List<BookingResponse> getBookingsByJobId(Long jobId) {
        return loadBookingPort.findByJobId(jobId).stream()
                .map(this::mapToResponse)
                .collect(Collectors.toList());
    }
