package com.flexjob.application.infrastructure.adapter.output.persistence.adapter;
...
import com.flexjob.application.infrastructure.adapter.output.persistence.mapper.ApplicationPersistenceMapper;
import com.flexjob.application.infrastructure.adapter.output.persistence.repository.ApplicationJpaRepository;
...
public class ApplicationPersistenceAdapter implements LoadApplicationPort, SaveApplicationPort {
    ...
    public Optional<Application> findById(ApplicationId applicationId) {
        return repository.findById(applicationId.getValue()).map(mapper::toDomain);
    }

    @Override
    public List<Application> findByJobId(Long jobId) {
        return mapper.toDomainList(repository.findByJobId(jobId));
    }

    @Override
    public List<Application> findByEmployeeId(Long employeeId) {
        return mapper.toDomainList(repository.findByEmployeeId(employeeId));
    }

    @Override
    public List<Application> findByEmployerId(Long employerId) {
        return mapper.toDomainList(repository.findByEmployerId(employerId));
    }
    ...
    @Override
    public boolean existsById(ApplicationId applicationId) {
        return repository.existsById(applicationId.getValue());
    }

    @Override
    public boolean existsByJobIdAndEmployeeId(Long jobId, Long employeeId) {
        return repository.existsByJobIdAndEmployeeId(jobId, employeeId);
    }

    @Override
    public Application save(Application application) {
        return mapper.toDomain(repository.save(mapper.toJpaEntity(application)));
    }
}
