package com.flexjob.application.domain.service;
...
public class ApplicationDomainService {
    ...
    public ApplicationMessage createMessage(MessageText messageText, Long senderId) {
        return ApplicationMessage.create(messageText, senderId);
    }

    public boolean canSendMessage(Application application, Long senderId) {
        return senderId.equals(application.getEmployeeId()) ||
               senderId.equals(application.getEmployerId());
    }
    ...
    public boolean isValidStatusTransition(ApplicationStatus from, ApplicationStatus to) {
        ...
        return switch (from) {
            case PENDING -> to == ApplicationStatus.ACCEPTED ||
