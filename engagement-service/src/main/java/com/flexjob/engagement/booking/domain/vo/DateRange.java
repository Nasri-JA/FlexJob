...
public class DateRange {
    ...
    public static DateRange of(LocalDate startDate, LocalDate endDate) { return new DateRange(startDate, endDate); }
    ...
    public long getDurationInDays() { return ChronoUnit.DAYS.between(startDate, endDate) + 1; }
    public boolean contains(LocalDate date) { return !date.isBefore(startDate) && !date.isAfter(endDate); }
    public boolean isInPast() { return endDate.isBefore(LocalDate.now()); }
    public boolean isInFuture() { return startDate.isAfter(LocalDate.now()); }

    @Override
    public String toString() { return String.format("DateRange(%s to %s, %d days)", startDate, endDate, getDurationInDays()); }
}
