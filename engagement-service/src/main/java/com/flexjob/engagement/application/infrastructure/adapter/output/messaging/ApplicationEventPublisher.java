package com.flexjob.engagement.application.infrastructure.adapter.output.messaging;
...
@Slf4j
public class ApplicationEventPublisher implements PublishApplicationEventPort {
    ...
    public void publishApplicationCreated(Application application) {
        try {
            ApplicationCreatedEvent event = ApplicationCreatedEvent.builder()
                    .applicationId(application.getId().getValue()).jobId(application.getJobId())
                    .employeeId(application.getEmployeeId()).employerId(application.getEmployerId())
                    .appliedAt(application.getAppliedAt()).build();
            kafkaTemplate.send("application-created", event);
            log.info("Published ApplicationCreatedEvent for {}", application.getId().getValue());
        } catch (Exception e) { log.error("Failed to publish event", e); }
    }
    ...
    public void publishApplicationStatusChanged(Application application, ApplicationStatus oldStatus) {
        try {
            ApplicationStatusChangedEvent event = ApplicationStatusChangedEvent.builder()
                    .applicationId(application.getId().getValue()).jobId(application.getJobId())
                    .employeeId(application.getEmployeeId()).employerId(application.getEmployerId())
                    .oldStatus(oldStatus.name()).newStatus(application.getStatus().name())
                    .changedAt(LocalDateTime.now()).build();
            kafkaTemplate.send("application-status-changed", event);
