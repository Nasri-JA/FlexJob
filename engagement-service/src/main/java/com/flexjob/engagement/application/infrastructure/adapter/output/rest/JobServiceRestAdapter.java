package com.flexjob.engagement.application.infrastructure.adapter.output.rest;
...
public class JobServiceRestAdapter implements JobServicePort {
    ...
    public JobInfo getJobById(Long jobId) {
        try {
            ...
            if (response != null) return new JobInfo(response.getId(), response.getEmployerId(), response.getTitle(), response.getStatus());
            return null;
        } catch (Exception e) { log.error("Failed to fetch job {}", jobId, e); return null; }
    }

    @Override
    public boolean jobExists(Long jobId) { return getJobById(jobId) != null; }
